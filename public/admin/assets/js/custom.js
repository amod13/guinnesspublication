...
|--------------------------------------------------------------------------
| - Handles showing/hiding of search filter form.
| - Remembers user's preference using localStorage.
| - Close button also hides the entire filter section and itself.
*/
document.addEventListener('DOMContentLoaded', function () {
    const filterCard = document.querySelector('#filterCard'); // main filter card section
    const searchBtn = document.querySelector('#searchBtnForToggleForm'); // top search toggle button
    const closeBtn = document.querySelector('.toggle-filter-btn-close'); // close (X) button inside form
    const storageKey = 'dealer_search_form_visible'; // unique key per module/page

    // 🟢 Restore previous visibility state from localStorage
    if (filterCard && localStorage.getItem(storageKey) === 'true') {
        filterCard.classList.remove('d-none');
        if (searchBtn) searchBtn.classList.add('active');
        if (closeBtn) closeBtn.classList.remove('d-none'); // show close button when visible
    } else if (filterCard) {
        filterCard.classList.add('d-none');
        if (searchBtn) searchBtn.classList.remove('active');
        if (closeBtn) closeBtn.classList.add('d-none'); // hide close button when hidden
    }

    // 🔄 Show filter form when clicking search button (manual close required)
    if (searchBtn && filterCard) {
    ...
    // ❌ Hide form + update localStorage + hide close button
    if (closeBtn && filterCard) {
