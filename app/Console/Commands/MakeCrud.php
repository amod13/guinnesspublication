...
class MakeCrud extends Command
{
    ...
    protected function removeCrudFiles($basePath, $module, $entity, $entityLower, $entitySnakePlural)
    {
        ...
        $this->removeRoutesFromWebFile($basePath, $module, $entity, $entityLower);
    ...
    protected function createController($basePath, $module, $entity, $entityLower)
    {
        ...
        {
            protected string \$viewPrefix = "{$moduleLower}::{$entityLower}.";
            ...
            {
                return \$this->updateOrderInternal(\$request, '{$entitySnakePlural}', 'id', 'display_order');
            }
        }
        PHP;
    ...
    protected function createViews($basePath, $module, $entity, $entityLower)
    {
            ...
            <div class="card">
                ...
                <div class="card-body">
                    <div class="p-4 border rounded">
                        @include('{$moduleLower}::{$entityLower}.partial._form')
                    </div>
                </div>
            </div>
        ...
        <form action="{{ isset(\$data['record']->id) ? route('{$entityLower}.update', \$data['record']->id) : route('{$entityLower}.store') }}" method="POST">
            ...
            <div class="mt-4 mt-4 d-flex justify-content-end">
                <x-form.submit-button :text="isset(\$data['record']->id) ? 'Update {$entity}' : 'Create {$entity}'" :class="'ms-2'" />
                <a href="{{ route('{$entityLower}.index') }}" class="btn btn-secondary btn-sm ms-2">Cancel</a>
            </div>
        </form>
        BLADE;
    ...
    protected function createRoutes($basePath, $module, $entity, $entityLower)
    {
        ...
        if (File::exists($webRoutesPath)) {
            ...
            // Add routes before the closing brace of the group
            if (strpos($existingContent, '//---------------------------ROLE HAS PERMISSION') !== false) {
                ...
                // Add before the last closing brace and parenthesis
                $existingContent = preg_replace(
                    '/(\s*)(\}\);\s*$)/s',
                    "$1{$routesContent}\n$1$2",
                    $existingContent
    ...
    protected function removeRoutesFromWebFile($basePath, $module, $entity, $entityLower)
    {
        $webRoutesPath = "{$basePath}/Routes/web.php";
