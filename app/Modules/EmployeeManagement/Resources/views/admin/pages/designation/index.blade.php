        ...
        <div class="col-lg-12">
            <div class="card shadow-sm border-0">
                <div class="card-body p-4">
                    ...
                    <div class="amd-soft-table-header d-flex justify-content-between align-items-center">
                        <h3 class="card-title mb-0 text-black">Designation</h3>
                        <a href="{{ route('designation.create') }}" class="amd-btn amd-btn-primary amd-btn-xs">
                            <i class="fa-solid fa-plus"></i> Add new column
                        </a>
                    </div>

                    <!-- Sub-header with rows selector, date range button, and search -->
                    <form method="GET" action="{{ route('designation.index') }}" class="row mb-3">
                                ...
                                @if (!empty($data['records']) && $data['records']->count())
                                    @foreach ($data['records'] as $index)
                                        <tr>
                                            ...
                                            <td class="d-flex">
                                                <a href="{{ route('designation.edit', $index->id) }}"
                                                    class="btn btn-sm me-2">
                                                    <i class="fas fa-edit"></i>
                                                </a>
                                                <form action="{{ route('designation.delete', $index->id) }}" method="POST"
