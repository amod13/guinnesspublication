...
@section('content')
    ...
    <div class="card shadow-sm border-0">
        {{-- Header Section --}}
        <x-table.top-header :title="'Gallery Category List'" :createRoute="route('gallery-category.create')" :createLabel="'Add New'" />

        <div class="card-body">
            <!-- Bulk Actions will be dynamically created by JS -->

            <!-- Table -->
            <div class="amd-soft-table-wrapper bulk-enabled" data-bulk-delete-url="{{ route('gallery-category.bulk.delete') }}">
                {{-- Filter --}}
                <x-table.filter :action="route('gallery-category.index')" :placeholder="'Search gallery categories'" />
                ...
                <table class="amd-soft-table" role="grid" aria-describedby="table-description">
                    ...
                    <tbody class="sortable-table" data-sort-url="{{ route('gallery-category.order') }}">
                            ...
                            <tr data-id="{{ $item['id'] }}" data-display-order="{{ $item['display_order'] ?? '' }}">
                                ...
                                <td name="bstable-actions">
                                    <div class="btn-group pull-right">
                                        {{-- Edit Button --}}
                                        <x-table.edit-button :id="$item['id']" :route="'gallery-category.edit'" />
                                        {{-- Delete Button --}}
                                        <x-table.delete-button :id="$item['id']" :route="'gallery-category.destroy'" />
                                    </div>
                                </td>
                            </tr>
                        @endforeach
