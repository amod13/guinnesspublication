...
@section('content')
    <div class="card shadow-sm border-0">
        {{-- Header Section --}}
        <x-table.top-header :title="'About Us List'" :createRoute="route('about-us.create')" :createLabel="'Add New'" />

        <div class="card-body">
            <!-- Bulk Actions will be dynamically created by JS -->

            <!-- Table -->
            <div class="amd-soft-table-wrapper bulk-enabled" data-bulk-delete-url="{{ route('about-us.bulkDelete') }}">
                {{-- Filter --}}
                <x-table.filter :action="route('about-us.index')" :placeholder="'Search about us'" />
                ...
                <table class="amd-soft-table" role="grid" aria-describedby="table-description">
                    ...
                    <tbody class="sortable-table" data-sort-url="{{ route('about-us.updateOrder') }}">
                            ...
                            <tr data-id="{{ $item['id'] }}" data-display-order="{{ $item['display_order'] ?? '' }}">
                                ...
                                <td name="bstable-actions">
                                    <div class="btn-group pull-right">
                                        <x-table.edit-button :id="$item['id']" :route="'about-us.edit'" />
                                        <x-table.delete-button :id="$item['id']" :route="'about-us.destroy'" />
                                    </div>
                                </td>
                            </tr>
                        @endforeach
