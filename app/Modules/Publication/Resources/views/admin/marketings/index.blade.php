...
@section('content')
    <form action="{{ route('marketings.index') }}" method="GET" class="position-relative">
        {{-- Close / Cross Icon --}}
    <button type="button" class="toggle-filter-btn-close position-absolute top-0 end-0 m-3 d-none amd-btn amd-btn-danger amd-btn-square-sm" aria-label="Close">
        ...
        <div id="filterCard" class="amd-fillter-card p-4 border rounded-4 mb-4 d-none">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-3">
                    <label for="statusSelect" class="form-label text-muted">Marketing Status</label>
    ...
    <div class="card shadow-sm border-0">
        {{-- Header Section --}}
        <x-table.top-header :title="'Marketings List'" :createRoute="route('marketings.create')" :column="true" :columnLabel="'Column Manage'" :tableId="'MaekrtingListTable'"
        ...
        <div class="card-body">
            ...
            <!-- Table -->
            <div class="amd-soft-table-wrapper bulk-enabled" data-bulk-delete-url="{{ route('marketings.bulk-delete') }}">
                {{-- Filter --}}
                <x-table.filter :action="route('marketings.index')" :placeholder="'Search Marketingss'" />

                <table class="amd-soft-table" role="grid" aria-describedby="table-description" id="MaekrtingListTable"
                            ...
                            <tr data-id="{{ $item['id'] }}" data-display-order="{{ $item['display_order'] ?? '' }}">
                                ...
                                <td name="bstable-actions">
                                    <div class="btn-group pull-right">
                                        <x-table.edit-button :id="$item['id']" :route="'marketings.edit'" />
                                        <x-table.delete-button :id="$item['id']" :route="'marketings.destroy'" />
                                    </div>
                                </td>
                            </tr>
                        @endforeach
