...
{{-- Search Filter --}}
    <form action="{{ route('authors.index') }}" method="GET" class="position-relative">
        <button type="button"
            class="toggle-filter-btn-close position-absolute top-0 end-0 m-3 amd-btn amd-btn-danger amd-btn-square-sm"
            aria-label="Close">
        ...
        <div id="filterCard" class="amd-fillter-card p-4 border rounded-4 mb-4 d-none">
            <div class="row g-3 align-items-end">
                ...
                <div class="col-md-6">
                    <label for="statusSelect" class="form-label text-muted">Author Status</label>
    ...
    <div class="card shadow-sm border-0">
        <x-table.top-header :title="'Authors List'" :createRoute="route('authors.create')" :column="true" :columnLabel="'Column Manage'" :tableId="'AuthorListTable'"
            :createLabel="'Add New'" :isSearch="true" :isDashboard="false" />

        <div class="card-body">
            <div class="amd-soft-table-wrapper bulk-enabled" data-bulk-delete-url="{{ route('authors.bulk-delete') }}">
                <x-table.filter :action="route('authors.index')" :placeholder="'Search Authors'" />

                <table class="amd-soft-table" role="grid" aria-describedby="table-description" id="AuthorListTable"
                    ...
                    <tbody class="sortable-table" data-sort-url="{{ route('authors.order') }}">
                            ...
                            <tr data-id="{{ $item['id'] }}" data-display-order="{{ $item['display_order'] ?? '' }}">
                                ...
                                <td name="bstable-actions">
                                    <div class="btn-group pull-right">
                                        <x-table.edit-button :id="$item['id']" :route="'authors.edit'" />
                                        <x-table.delete-button :id="$item['id']" :route="'authors.destroy'" />
                                        <a href="{{ route('authors.show', $item['id']) }}">
                                            <button type="button" class="btn btn-sm btn-default" title="View Details">
