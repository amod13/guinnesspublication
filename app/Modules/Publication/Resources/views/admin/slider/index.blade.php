...
@section('content')
    ...
    <div class="card shadow-sm border-0">
        {{-- Header Section --}}
        <x-table.top-header :title="'Slider List'" :createRoute="route('slider.create')" :createLabel="'Add New'" />

        <div class="card-body">
            <!-- Bulk Actions will be dynamically created by JS -->

            <!-- Table -->
            <div class="amd-soft-table-wrapper bulk-enabled" data-bulk-delete-url="{{ route('slider.bulk.delete') }}">
                {{-- Filter --}}
                <x-table.filter :action="route('slider.index')" :placeholder="'Search sliders'" />
                ...
                <table class="amd-soft-table" role="grid" aria-describedby="table-description">
                    ...
                    <tbody class="sortable-table" data-sort-url="{{ route('slider.order') }}">
                            ...
                            <tr data-id="{{ $item['id'] }}" data-display-order="{{ $item['display_order'] ?? '' }}">
                                ...
                                <td name="bstable-actions">
                                    <div class="btn-group pull-right">
                                        {{-- Edit Button --}}
                                        <x-table.edit-button :id="$item['id']" :route="'slider.edit'" />
                                        {{-- Delete Button --}}
                                        <x-table.delete-button :id="$item['id']" :route="'slider.destroy'" />
                                    </div>
                                </td>
                            </tr>
                        @endforeach
